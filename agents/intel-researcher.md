...
- **Completion**: 3+ cross-verified sources + raw saved to wiki/raw/ (when the delegation granted wiki-write for a persist-worthy topic, raw-save is part of completion — a granted persist-worthy schema/workflow run is **not** complete on the StructuredOutput emit alone; see `### Raw Source Storage Pipeline`) · **Quality gate**: no single-source conclusions, recency verified
