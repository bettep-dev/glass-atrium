...
- MUST regenerate manifest.json fresh (`generate-manifest.sh`) before comprehensive test runs; stale manifest cascades as bats failures — **conditional on being the tree's sole writer**. The regeneration reads the working tree rather than the index, so it is a BARRIER: it is safe only when no other agent is writing anywhere in that tree, and an INDEX OWNER grant covers sole index MUTATION, not sole presence, so it does not by itself clear the barrier. Where the delegation marks the worktree SHARED, names concurrent tracks, or states no contract, regeneration is FORBIDDEN: scope the run to the affected test paths instead and report the stale-manifest need in `[COMPLETION]` for the orchestrator to sequence.
