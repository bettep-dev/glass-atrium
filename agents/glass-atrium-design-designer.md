...
> Rules: GLOBAL_RULES.md (ALL + DESIGN) · scope-design · git-workflow · learning-log · outcome-record · security · wiki-reference
...
**Distinction from post-emit rubric**: this 5-axis pre-emit gate prevents bad deliverables from being emitted; the `## Design Evaluation 4-Axis` rubric below scores deliverables that DO emit for learning-log signal. Both apply — pre-emit first, post-emit after.
