...
- **Cycle-count limit**: hard stop after 5 completed cycles — emit [COMPLETION] `needs_context` with the checkpoint payload (per Work-unit checkpoint above); at cycle 4 with ≥70% budget consumed, checkpoint immediately (staying under budget beats reaching cycle 5). The Error Recovery "max 3 cycles" stays the tighter bound on the all-rejected restart path; this 5-cycle stop caps total cycles across the investigation.
