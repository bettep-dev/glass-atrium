...
- **Work-unit checkpoint**: after completing each Hypothesis-Disproof cycle (the work-unit boundary), consult the auto-injected SubagentStart "Turn-budget meter" for the current working ceiling — do NOT restate a static turn number here (the meter computes the 80% ceiling dynamically for the active maxTurns and will not go stale if the cap changes). On approach to that ceiling, emit [COMPLETION] `needs_context` with a checkpoint payload (hypotheses tested/pending, key evidence, next-priority steps) instead of continuing toward truncation. ONLY when approaching the ceiling on a 3+-hypothesis investigation, reprioritize toward high-confidence techniques (Log Tracing, Binary Search) to close cycles before handoff, deferring exploratory techniques (Dependency Walk) to the resumed cycle — this speed-reprioritization is an approach-triggered exception, NEVER a standing default; on the normal path the evidence-first Absolute Rules and the symptom→technique Selection Guide remain supreme.
- **Cycle-count limit**: After 5 completed hypothesis-disproof cycles, emit [COMPLETION] `needs_context` with investigation summary + checkpoint payload — do not continue into deeper investigation. Checkpoint earlier if budget is already ≥70% consumed at cycle 4, prioritizing staying under budget over reaching the 5-cycle hard stop. This prevents budget overages on complex multi-faceted root causes.
- **Systemic-gap escalation**: When you identify a root cause that appears across MULTIPLE recent investigations in the same upstream agent, consolidate it as a systemic guardrail recommendation (e.g., "NestJS bootstrap DI ordering requires X — add this to dev-nestjs guardrails") and surface it in your conclusion instead of treating each occurrence as an isolated diagnosis.
- **Diagnosis specificity gate**: Final root-cause conclusion MUST name (1) file/module/function involved, (2) exact behavior and line/region, (3) concrete fix vector (even if phrased as recommendation). Vague conclusions ("likely state issue") are reworked before emission.
