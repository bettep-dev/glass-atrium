  ...
  - **Content-driven escalation (apply the matching visual only)**:
    ...
    - **Pre-drawing decision core (delivered copy — apply to EVERY Mermaid block, not only the first)**: Type → Direction → Budget → Preset → semantic-role `classDef` → Layout.
      - **Type — the adopted set is closed at seven**: `flowchart` · `sequenceDiagram` · `stateDiagram-v2` · `erDiagram` · `classDiagram` · `gitGraph` · C4 (`C4Context` / `C4Container` / `C4Component`). Everything else — quadrantChart, radar, pie, timeline, journey, mindmap, sankey, xychart, gantt, block — is EXCLUDED: express that content as a table or prose. Renderable by Mermaid is not the same as adopted.
        - SoT for that adopted/excluded split: `monitor/src/server/clauded-docs/diagram-types.json`, which the HTML validator `JSON.parse`-loads at module init; six prose copies restate it, this one included, and the JSON governs every one — a copy stating a different count is the drift, never the JSON.
