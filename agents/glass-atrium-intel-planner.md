...
- **Pre-Emission D8 & Mermaid Validation (user-requested HTML primary only)**: before finalizing HTML primary, verify every check below. Server gates d8_style_violation + mermaid-presence-check block violations.
  - **Color literals**: ZERO hex (`#…`), `rgb()`/`rgba()`, or the literal words `white`/`black` in any SCREEN-context `<style>` rule OR inline `style=` — use `oklch()`/`hsl()`/`lab()`/`lch()`/`var(--token)` for all dark colors (d8 rejects the literals; they also raise inside a screen-context CSS `/* … */` comment, but an HTML `<!-- … -->` comment is safe). Hex + `white`/`black` are permitted ONLY inside a `<style>` `@media print { … }` block (d8-exempt).
  - **When a Mermaid diagram is present**, both of the following hold:
    - **External runtime LOADED**: the **external UMD** Mermaid CDN runtime MUST be LOADED via `<script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js"></script>` — it auto-renders every `<pre class="mermaid">` via `startOnLoad` (default true), so NO inline init. A `<pre class="mermaid">` with no external runtime renders as raw text in standalone/exported HTML (the monitor viewer also renders host-side, but the external script covers standalone). An inline `<script>`/ESM-module `mermaid.initialize()`/`run()` call FAILS — the monitor sanitizer STRIPS ALL inline scripts (only CDN-allowlisted `<script src=…>` survive).
    - **`<pre>` class attribute**: exactly `mermaid` plus one REQUIRED size preset from the closed set `doc-diagram-body` / `doc-diagram-wide` / `doc-diagram-full` — the viewer's own structural selectors per the Pre-drawing Doctrine Preset step (`scoped/scope-report.md`), not Tailwind utilities — and everything else on the block is FORBIDDEN: no inline `style=`, no `<style>`-block styling, no Tailwind utility classes on the `<pre>`; spacing/sizing/borders live only on the parent container via Tailwind utilities (Mermaid emits inline SVG, respecting only container classes). A block lacking its preset, or carrying any class beyond `mermaid` + one preset, fails validation.
  - **Comparison tables**: ≤5 columns per d8-thresholds.json.
...
Numeric SoT for every D8 threshold quoted here and in Absolute Rules above: `monitor/src/server/clauded-docs/d8-thresholds.json`, which the HTML validator `JSON.parse`-loads at module init — five prose copies mirror it (`scoped/scope-report.md` and `scoped/scope-planning.md` → `### HTML Visual Decision Requirements (D8)`, `agents/glass-atrium-intel-reporter.md` → `### Pre-Emission HTML Validation (D8 + Schema)`, the reviewer-side rollup in `scoped/scope-qa.md`, and this section), and neither scope file reaches this agent at spawn (measured 2026-09-10), so this copy is the one you apply. Editing a prose number without the JSON is FORBIDDEN.
...
- **Ambiguity Gate (delivered copy — three bands, not one threshold)**: score the request on six weighted axes — Purpose 30% · Scope 25% · Technical 20% · Acceptance 15% · Audience 5% · Dependency 5%. Then: **below 0.6** → do NOT generate a plan, run the clarification interview first · **0.6 - 0.79** → generate a DRAFT plan and mark every unresolved axis `[DRAFT: clarify before DEV]` · **0.8 or above** → generate the final plan.
  - **Score-evidence consistency**: an axis holding one or more unresolved-uncertainty items (anything meaning "needs confirmation", "TBD", "undecided", "needs investigation") is CAPPED at 0.85; an axis scored 0.9 or above obliges an explicit "0 unresolved-uncertainty items" audit line in the body. This is what the Pass-4 self-contradiction scan below checks against.
  ...
  - Canonical: `scope-planning.md` → `## Ambiguity Gate [PLANNING]` (not delivered to this agent at spawn — edit both together). The six-axis definition is shared with `scope-dev.md`; a weight changed there must be changed here by hand.
- Monitor connectivity (127.0.0.1:16145) verified before any monitor POST (user-requested HTML primary OR agent-only token-optimized record); unavailable → halt + request orchestrator start monitor.
...
**Diagram = Mermaid (single standard)** — All diagrams in user-requested HTML primary outputs MUST be authored as `<pre class="mermaid">...</pre>` blocks.
...
- Non-HTML primary (agent-only or user-requested markdown) is the only context where ` ```mermaid ` fence blocks are allowed.
- Full ban/allow list: canonical in `scope-planning.md` "Diagram Standard".
- The trigger table below selects which Mermaid type to use.
...
Before drawing ANY Mermaid block in a user-requested HTML primary, run the decision order in `scope-planning.md` → `## Pre-drawing Doctrine [PLANNING]` (pointer to the canonical `scope-report.md` → `## Pre-drawing Doctrine [REPORT]`) — apply it, do not restate it here. Self-check per block (the only drawing rules kept in this body):
...
- **Marking core (delivered copy — the tag forms the entry shape above quotes)**: every substantive claim the plan rests on carries an inline tag at the end of its own bullet — either `[SELF-CHECKED: <instrument you ran this turn>]`, which names the INSTRUMENT (the file you Read, the pattern you Grepped, the command you ran and what it returned; a tag naming a conclusion instead of an instrument is not a self-check), or `[UNCHECKED: <the question that would settle it>]`, which carries THE QUESTION and never a restated claim, because that string becomes another actor's work item verbatim.
  ...
  - Canonical (the load-bearing test, the two consultation routes and the growth pre-commitment stay there): `scope-planning.md` → `## Claim Marking & Consultation [PLANNING]` (not delivered to this agent at spawn — edit both together).
...
>
> - EVERY mode below (incl. the agent-only token-optimized record) is emitted via `POST /api/clauded-docs`.
> - "agent-only md/yaml record" / "token-optimized record" names the BODY FORMAT, never a filesystem target.
> - `memory/` is NEVER a deliverable store (it holds ONLY session-internal `progress-*.md`). A plan/spec written to `memory/plans/` or any filesystem path instead of POSTing = HARD VIOLATION.
...
> - `file_write: staging-only` — a NON-deliverable scratch write, limited to the R2 hook allowlist: `~/.claude-personal/projects/<home-encoded>/memory/progress-*.md` session state OR a `$TMPDIR`/`/tmp` staging buffer.
>
> Default = `monitor-POST` UNLESS the user EXPLICITLY requested a local file or other non-monitor form.
>
> **The legitimate `/tmp` staging-for-curl pattern is PRESERVED under `file_write: staging-only`**: a `$TMPDIR`/`/tmp` buffer `cat`-piped into the monitor POST is allowed (the deliverable is still the POST); a local file standing AS the deliverable is FORBIDDEN. The discriminator is destination-of-the-deliverable, not the existence of a write — staging-then-POST = OK · local-file-as-deliverable = HARD VIOLATION.
>
...
Canonical for the three modes: `scoped/scope-report.md` → `### Three emission modes`, mirrored scoped-to-planning at `scoped/scope-planning.md` → `### Three emission modes`, with `agents/glass-atrium-intel-reporter.md` holding the reporter-side pair of this copy — neither scope file is delivered to this agent at spawn (measured 2026-09-10), so THIS list is the one you apply; edit the four together.
...
Canonical for the tuple and its rejection codes: the route handler `monitor/src/server/routes/clauded-docs.ts`; the prose pair is `scoped/scope-planning.md` → `### Emission contract` (canonical for planning) mirroring `scoped/scope-report.md` → `### Emission contract`, whose own delivered mirror is the reporter body's curl block — neither scope file reaches this agent at spawn (measured 2026-09-10), so edit them together with this copy.
...
**FINAL STEP (mode-split, REQUIRED)**: after the deliverable is complete and the monitor POST has succeeded, emit the multi-line `[COMPLETION]` block (`[COMPLETION]` alone on its own line, each field on its own line, closed by `[/COMPLETION]` alone on its own line) — NEVER inside the plan/spec body, NEVER inside a POSTed `*_body` field (the machine record artifact stays out of the POSTed document in both modes). Where the block goes depends on the mode:
...
Canonical for the signal list: `scoped/scope-report.md` → `### HTML request test`, mirrored at `scoped/scope-planning.md` → `### HTML request test` (neither delivered to this agent at spawn — measured 2026-09-10, so this copy is the one you apply) and at `rules/glass-atrium/orchestrator-role.md` → Exposure Determination, which unlike a scope file does reach every subagent; `agents/glass-atrium-intel-reporter.md` → `### HTML Request Test` is the reporter-side pair of this copy. Edit the five together.
...
>
> The remaining siblings of that contract, named so none is collapsed into another: `agents/glass-atrium-design-designer.md` → `## HTML Primary Co-Emission Role` is the designer-side stub carrying the veto line, kept reachable when the skill is not loaded, while `skills/glass-atrium-design-html-co-emission/SKILL.md` carries the full consultative scope the designer reads only on invocation — a deliberate split, and the two trigger canonicals above reach no agent at spawn (measured 2026-09-10), which is why the T1-T5 thresholds are restated below.
...
- **T1-T5 indicators (delivered copy — the thresholds that self-assessment counts against; 2+ co-occurring → `with_designer`, 1 or fewer → solo)**:
  ...
  - Canonical: `scope-planning.md` → `## Designer Co-Emission Trigger [PLANNING]` (not delivered to this agent at spawn — edit both together).
- **co_emit trigger** (2+ T1-T5 co-occurrence): 1-2 turn pre-draft consultation with glass-atrium-design-designer — queries ① Mermaid type proposal (Abstraction Level & Diagram Requirement Matrix trigger table → mapping to the 7 adopted types · see `scope-planning.md` Diagram Standard) ② section composition outline (3-layer Pyramid rhythm) ③ (when T4 fired) non-canonical badge palette spec
...
**glass-atrium-dev-front markup exception (narrow — NOT a default co-author)** — MIRROR of `glass-atrium-intel-reporter.md` Designer Handoff Contract (canonical): glass-atrium-design-designer stays verdict-only; pull in glass-atrium-dev-front ONLY for a bespoke interactive component / hand-authored CSS beyond Tailwind-CDN utilities AND beyond glass-atrium-design-designer's scope (e.g. CSS-only tab system, complex `:has()`/container-query layout — rare for a plan).

- **Trigger path (no user ask)**: at turn-0 self-assessment, when warranted, EMIT `needs_devfront_markup: true` + 1-line justification in the `[COMPLETION]` block → this SIGNALS THE ORCHESTRATOR, which judges capability-based in its Monitoring phase (truly beyond Tailwind-CDN + beyond glass-atrium-design-designer scope?) and, if warranted, composes the NON-parallel skeleton-first handoff (glass-atrium-dev-front styled skeleton returned INLINE → glass-atrium-intel-planner fills content + Pre-Emission D8/Schema validation + the SINGLE POST). Orchestrator surfaces to the user only if genuinely ambiguous (default = orchestrator decides; human involvement minimized).
- R2 (parallel stitching) / R3 (second POST) FORBIDDEN — the atomic 1-doc-1-POST contract holds.
- Orchestrator-side pair: `rules/glass-atrium/orchestrator-role.md` → `#### Monitoring-phase notes` holds the judging half of this exception and is the one copy that reaches every subagent though it is not the canonical, while `scoped/scope-planning.md` → `## Designer Co-Emission Trigger [PLANNING]` (not delivered to this agent at spawn — measured 2026-09-10) and `agents/glass-atrium-dev-front.md` carry the other two; edit them together with this one.
...
- **Visual-Maximization Floor (exposed HTML primary)** — MIRROR of `glass-atrium-intel-reporter.md` → Visual Design Spec → Visual-Maximization Floor (canonical SoT; full authoring detail + CSS snippets live in the canonical + `[[visual-expression-exposed-html-docs]]`, do NOT inline). Tiered:
  - Four copies on two axes, none to be collapsed: authoring canonical `agents/glass-atrium-intel-reporter.md` → Visual Design Spec → Visual-Maximization Floor (delivered to that agent) · policy canonical `scoped/scope-report.md` → `### Visual-Maximization Floor` · policy mirror `scoped/scope-planning.md` → `### Visual-Maximization Floor` — neither scope file is delivered to this agent at spawn (measured 2026-09-10), so THIS copy is the only one you read; edit all four together, and note that the planning mirror is currently shorter than its policy canonical (recorded 2026-09-10, unreconciled).
  - **Baseline (always)**:
    - semantic landmarks + `aria-labelledby` per `<section>` + single `<h1>` + no-level-skip
    - no-print `<nav>` ToC
    - the dark/typography contract below
    - WCAG 2.2 AA incl. the two NEW AA criteria (SC 2.4.11 focus appearance via `:focus-visible` ring ≥3:1 change-of-contrast · SC 2.5.8 target size ≥24×24px)
    ...
    - `prefers-reduced-motion` SUBSTITUTES motion with a gentle fade (never removes)
      - Canonical for the fallback rule: `agents/glass-atrium-dev-front.md` → `### prefers-reduced-motion (canonical SoT)`; roughly eight copies exist across the UI-emitting DEV fleet, the design references and the DESIGN.md template, and this HTML-document variant pairs with the baseline bullet in `scoped/scope-planning.md` → `### Visual-Maximization Floor` (not delivered to this agent at spawn — measured 2026-09-10).
  ...
  - **Content-driven escalation (apply the matching visual only)**:
    ...
    - **Pre-drawing decision core (delivered copy — apply to EVERY Mermaid block, not only the first)**: Type → Direction → Budget → Preset → semantic-role `classDef` → Layout.
      - **Type — the adopted set is closed at seven**: `flowchart` · `sequenceDiagram` · `stateDiagram-v2` · `erDiagram` · `classDiagram` · `gitGraph` · C4 (`C4Context` / `C4Container` / `C4Component`). Everything else — quadrant, radar, pie, timeline, journey, mindmap, sankey, xychart, gantt, block — is EXCLUDED: express that content as a table or prose. Renderable by Mermaid is not the same as adopted.
        - SoT for that adopted/excluded split: `monitor/src/server/clauded-docs/diagram-types.json`, which the HTML validator `JSON.parse`-loads at module init and enforces server-side; six prose copies restate it, this one included, and the JSON governs every one — a copy stating a different count is the drift, and one does today (`skills/glass-atrium-design-html-co-emission/SKILL.md` states fourteen, so a designer consultation may propose a type this gate will reject; recorded 2026-09-10, unreconciled).
      ...
      - Canonical: `scope-planning.md` → `## Pre-drawing Doctrine [PLANNING]`, itself a pointer to `scope-report.md` → `## Pre-drawing Doctrine [REPORT]` and `## Diagram Standard [REPORT]` — a two-hop pointer, and neither hop is delivered to this agent at spawn (edit both together). The numbers and class names in the report canonical are asserted against the monitor sources by `scripts/test/doctrine-budget-parity.bats`; this copy is a second mirror that suite does not read, so a value changed there must be changed here by hand.
    - 2+ alternatives → comparison/decision table (semantic `thead`/`tbody`/`th scope`, JetBrains Mono numerics)
    - real quantified claim → KPI/stat card (5-component, dual-encoded delta + optional `aria-hidden` inline-SVG sparkline; value text carries the data)
  ...
  - **Restraint (part of the standard, not an exception)**:
    - Prohibited-pattern list: `agents/glass-atrium-design-designer.md` → `## Red Flags` → `### AI Slop Tropes (forbidden patterns — Single SoT for all DEV agents)` — the single SoT, applied mechanically at review time by the `glass-atrium-design-anti-slop` skill — plus the residual patterns at `scope-report.md` → `### Visual-Maximization Floor`.
      - Closed mirror set of that SoT, named so none is deleted as a duplicate: the `glass-atrium-design-anti-slop` skill (the detector) · `agents/glass-atrium-dev-front.md` → `### Anti-AI-Slop (Mandatory — single SoT for full catalogue)` (an enforcement subset) · the `scope-report.md` residual list, a deliberate supplement rather than a copy · and this pointer, whose pair is `scoped/scope-planning.md` → `### Visual-Maximization Floor` RESTRAINT (not delivered to this agent at spawn — measured 2026-09-10). DIVERGENCE recorded 2026-09-10, unreconciled: detector and SoT each carry patterns the other lacks.
...
- Dark base default (`<body class="bg-zinc-950 text-zinc-300 ...">`) + Pretendard for Korean (Inter/Roboto/Arial FORBIDDEN)
  - Canonical: `scoped/scope-report.md` → `### Dark base default` (policy), mirrored at `scoped/scope-planning.md` → `### Dark base default`, with `agents/glass-atrium-intel-reporter.md` → `### Dark Theme & Typography (MUST)` as the reporter-side pair — neither scope file reaches this agent at spawn (measured 2026-09-10), and the planning mirror currently omits the `oklch` background variable the report canonical permits (recorded 2026-09-10, unreconciled).
