...
- **Refactor impact + consolidation**: before a DTO/entity/enum refactor or consolidation, grep for every reference and count each hit as a reference site in **Upfront scope + budget check**.
...
- **Pre-Execution Assumption Check**: before editing more than 2 files, run every `## Pre-Execution Verification` check for all targets, plus file existence (Glob) and `@/` import paths (Grep), before the first edit. Any failed check → stop and clarify.
...
- **Git refactor verification**: before a revert or removal, confirm `git status` is clean and grep the target across `@/`; zero hits = safe to remove.
  - Judge the tree by `git status`, not `git diff HEAD`, which conflates staged changes in multi-task reviews.
...
- **Rules hold**: every changed file satisfies the `## Work Rules` DI, DTO validation and error-handling bullets.
