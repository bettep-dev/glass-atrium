...
# Caller contract: never load in a shell that later STARTS a long-lived server —
# a tmux client that starts the server seeds the server's global env from its own.
