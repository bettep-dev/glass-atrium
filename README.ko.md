...
각 하위 작업은 키워드가 아니라 역량으로 라우팅되어(키워드·별칭 매칭 금지) 누가 무엇을 하는지 정확히 파악됩니다. 각 에이전트가 구조화된 완료 기록을 돌려주면, 오케스트레이터가 의도와 결과가 일치하는지 검증합니다. 실패한 서브에이전트는 실패 복구 루프(재시도 → 폴백 → 디버거 에스컬레이션)로 처리되며, 스폰 깊이·동시성 상한·에이전트별 예산이 강제되어 어떤 위임도 폭주하지 않습니다. 서로 겹치지 않는 독립적인 하위 작업은 사용자가 요청하지 않아도 기본으로 병렬 실행됩니다.
...
4. **전문 에이전트가 작업을 수행합니다** — SubagentStart 훅이 주입한 자기 범위 규칙과 강제된 스폰·턴·도구 예산 안에서. 긴 작업은 예산 한계에서 강제 종료되는 대신, 진행 상황을 기록하고 재개 가능한 상태로 정지합니다.
...
새 버전이 나오면 아트리움 모니터 대시보드에 새 버전 번호와 함께 **Update available** 배지가 나타납니다. 이 배지를 한 번 클릭하면 설정과 데이터는 그대로 둔 채 업데이트가 자동으로 적용되고, 끝날 때까지 진행 상황이 화면에 표시됩니다. 처음 설치할 때 쓴 한 줄 명령을 다시 실행해도 언제든 안전하게 제자리에서 업데이트되며, 터미널에서는 `glass-atrium update` 명령도 쓸 수 있습니다.
