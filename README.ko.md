...
오케스트레이터가 요청 하나를 받아 전문 에이전트 팀에게 분해·위임하고 그 결과를 종합합니다. 모든 비용과 결과, 에이전트 이벤트는 유리 벽 너머를 들여다보듯 실시간 모니터 대시보드에 드러나며, 시스템은 반복되는 실패로부터 스스로 지침을 고쳐 나갑니다 — 프롬프트를 손볼 필요가 없습니다.
...
메뉴에서 **Uninstall** 을 선택하세요. 설치된 심링크를 제거하고 GA 데이터베이스를 드롭해 기존 Claude 시스템에서 아트리움을 깨끗이 떼어내며, 사용자 소유 파일과 잔여물은 남기지 않습니다. 단, **데이터베이스는 백업 없이 삭제되며**, 재설치하면 새로 만들어집니다.
