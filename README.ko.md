...
각 하위 작업은 키워드가 아니라 역량으로 라우팅되어(키워드·별칭 매칭 금지) 누가 무엇을 하는지 정확히 파악됩니다. 각 에이전트가 구조화된 완료 기록을 돌려주면, 오케스트레이터가 의도와 결과가 일치하는지 검증합니다. 실패한 서브에이전트는 실패 복구 루프(재시도 → 폴백 → 디버거 에스컬레이션)로 처리됩니다. 에이전트별 턴 예산은 하네스가 스폰 시점에 하드 캡으로 적용하고, 동시성 상한은 워크플로 엔진 런타임이 겁니다. 스폰 깊이는 정책 숫자가 아니라 구조로 묶여 있습니다 — 어떤 에이전트 프론트매터도 재스폰 도구(Agent/Task/Workflow)를 부여받지 않고, 그 도구 목록은 스폰 시점에 동결되기 때문입니다. 서로 겹치지 않는 독립적인 하위 작업은 사용자가 요청하지 않아도 기본으로 병렬 실행됩니다.

이 통제의 기계적 층은 실제 코드입니다 — PreToolUse 훅 파이프라인, `enforce-harness-critical.sh`의 하네스 임계 경로 차단, 그리고 스폰 시점 도구 동결. 나머지는 규율(honor-system) 층입니다 — 위임 재시도·폴백·에스컬레이션, 그리고 `MAX_DEPTH=2`라는 정책 숫자와 깊이 카운팅. 저장소의 규칙 파일들도 스스로 그렇게 적어 둡니다.
...
대화형 메뉴가 열리면 **Install** 을 선택하세요 — 설치가 끝나면 의존성과 데몬이 자동으로 설정·실행되고, 대시보드는 `http://127.0.0.1:16145` 에서 접근할 수 있습니다. 설치는 헬스 게이트를 통과해야 끝나며, 이후에도 `glass-atrium doctor` 또는 모니터 **System map** 화면의 **System health** 스트립에서 상태를 다시 확인할 수 있습니다.
