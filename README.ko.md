...
세션 시작 시 전역으로 로드되고, Claude가 작업에 맞춰 알아서 활성화합니다. 코드·디자인 규약, 운영·검증 게이트, 위키·웹 도구를 다루며, 일부는 모니터 신호(에이전트 삭제 결과 등)에 반응해 발동합니다. 어느 스킬이 도는지 알 필요는 없습니다 — 하니스가 알아서 골라 호출합니다.
