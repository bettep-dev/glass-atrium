...
업데이트는 처음 설치할 때 쓴 한 줄 명령을 다시 실행하면 됩니다. 언제 다시 실행해도 안전합니다 — 이미 최신 버전이면 아무것도 하지 않고, 새 버전이 나와 있으면 설정과 데이터는 그대로 둔 채 파일만 제자리에서 바꿔 놓습니다. `glass-atrium update` 명령을 써도 됩니다. 최신 버전을 내려받아 무엇이 바뀌는지 먼저 보여 주고, 한 번 확인을 받은 뒤에 적용합니다.
