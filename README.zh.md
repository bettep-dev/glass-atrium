...
每个子任务都按能力而非关键字进行路由（禁止关键字/别名匹配），因此谁负责什么都被精确掌握。每个智能体都会返回一份结构化的完成记录，供编排器核对意图与结果是否一致。失败的子智能体交由失败恢复循环处理（重试 → 回退 → 上报给调试器）。各智能体的轮次预算由框架在派生（spawn）时刻作为硬上限施加，并发上限则由工作流引擎在运行时施加。派生深度不是靠一个策略数字，而是靠结构被约束住的——没有任何智能体的 frontmatter 被授予再次派生的工具（Agent/Task/Workflow），而这份工具清单在派生时刻即被冻结。互不重叠的独立子任务，即使你不主动要求，也会默认并行执行。

这套掌控的机械层是真实的代码——PreToolUse 钩子流水线、`enforce-harness-critical.sh` 对框架临界路径的写入拦截，以及派生时刻的工具冻结。其余部分属于自律（honor-system）层——委派的重试/回退/上报，以及 `MAX_DEPTH=2` 这个策略数字及其深度计数。仓库里的规则文件自己也是这么写的。
