...
每个子任务都按能力而非关键字进行路由（禁止关键字/别名匹配），因此谁负责什么都被精确掌握。每个智能体都会返回一份结构化的完成记录，供编排器核对意图与结果是否一致。失败的子智能体交由失败恢复循环处理（重试 → 回退 → 上报给调试器），而强制生效的派生（spawn）深度、并发上限与各智能体预算，让任何一次委派都无法失控。
...
4. **专业智能体执行工作**——在 **SubagentStart 钩子**所注入的自身范围规则，以及强制生效的派生/轮次/工具预算之内完成。
...
- **分层规则系统** — 用一份明确的合规矩阵，把全局宪章（`agents/GLOBAL_RULES.md`）、核心横切规则（`rules/`）与按范围划分的规则（`scoped/`）绑在一起。
- **生命周期钩子流水线** — 一组钩子脚本（`hooks/`），在每一个工具边界上机械地强制执行密钥、危险命令、预算与结果记录。
- **自我改进循环** — autoagent 守护进程（`autoagent/`）把累积的结果记录与纠正信号转化为智能体指令补丁，只有安全的部分才会自动应用。应用之前会先把原文另存一份，一旦出问题便原样回滚。
- **Atrium Monitor** — 基于 Fastify 5 + Prisma 7 + React 18、共 10 个界面的实时仪表盘（`http://127.0.0.1:7842`）。
...
交互式菜单打开后，请选择 **Install**——安装完成后，依赖与守护进程会自动配置并运行，仪表盘可在 `http://127.0.0.1:7842` 访问。
...
请求文档、报告、摘要或参考资料时，会委派给 **intel-reporter**；请求企划、需求定义或任务分解时，则委派给 **intel-planner**。
...
正文（系统提示词）由 **meta-prompt-engineer** 撰写，而登记本身交给专用的 CLI 处理。概括来说，你只需请求「添加一个」即可——扩展还是新建哪个更合适，由编排器判断并给出建议；确需新建时，则会征求你的批准。
