...
**Tier 1 — Auto (default)**:
- Regular instruction improvements (single agent, 5+ occurrences, 1-line change basis — former Auto criteria retained)
- daemon-apply.sh auto-applies (unchanged)
...
**Signal origin (agent-emitted, semantic — regex is legacy fallback only)**: the correction signal now originates from the agent's `[COMPLETION]` emit. When the user's latest message corrected/rejected/asked-to-redo the work, the agent judges this SEMANTICALLY in ANY language and emits `revision_count` ≥ 1 + `evaluative_signal: -1` + `directive_hint` (per `core-outcome-record.md` Correction-emission rule). track-outcome.sh keys `core.correction_signals` on these emitted fields; the former Korean/English keyword regex on the user message is DEMOTED to a legacy fallback that fires only when NO correction field was emitted. The hook's prior-outcome accumulation (prior+1) remains the authoritative numeric delta — the agent fields are the trigger, not the count.

**Procedure**:
1. Increment revision_count (transient session signal) — the hook's prior+1 accumulation is authoritative; the agent emit is the trigger
...
3. **No auto-persistence to user-facing memory**: a correction signal NEVER auto-writes `feedback_*.md` / `MEMORY.md` in the personal memory dir. The daemon's `directive_hint`→`feedback_*.md` auto-clustering path is gated-off by default in code (learning-aggregator.py) — neutralized via a two-factor (env var truthy AND a filesystem safety-marker) authorization that is fail-safe-to-OFF (env-on without the marker stays OFF), NOT deleted; the path is present and re-enableable only by an authorized operator. Internal CTM/EPM accumulation (`memory/core-learning-log.md`) may still record the distilled pattern, but writing user-facing memory requires an explicit user instruction to remember — see Long-Term Memory Write-Gate condition 4.
...
- **explicit user instruction to remember (OVERRIDING gate)**: a user-facing memory write fires ONLY when the user explicitly instructed it to be remembered (e.g. `기억해` / `이거 기억해둬` / "remember this" / "save this to memory", judged semantically in ANY language). The main session MUST NOT proactively/automatically persist memory, and the daemon MUST NOT auto-generate `feedback_*.md` from clustered `directive_hint`s — both auto-paths are FORBIDDEN (the daemon path is gated-off by default in learning-aggregator.py via a two-factor env + safety-marker authorization, fail-safe-to-OFF — present but re-enableable only by an authorized operator, NOT deleted). This condition is necessary on its own: the three conditions below stay required but are no longer SUFFICIENT without it.
