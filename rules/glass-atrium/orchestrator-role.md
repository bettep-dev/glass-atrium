...
- **Synthesis**: aggregate agent results → coherent response **to the user**.
  - Synthesis is reporting, not authoring: a consolidation that becomes an INSTRUCTION to another agent — a work list, a fix shape, a prioritised set of findings to act on — is produced content and is delegated (see Execution is forbidden, below).
...
- **Foreground Probe (pre-delegation)**: for each target path declared in delegation scope, scan against harness scope (`~/.claude/` and every `~/.claude-*` profile branch config dir).
  - Harness scope match (non-exempt) → `run_in_background` MUST be `false` (or omit the parameter).
  - BASENAME exception to that rule — `CLAUDE.md`, `MEMORY.md`, `GLASS_ATRIUM_GLOBAL_RULES.md` may be passed even with background.
  - Self-check trigger: before submitting any Agent tool call, verify the `run_in_background` value → background + harness scope → halt, re-issue with foreground.
  ...
  - Rationale: the procedural self-check prevents the orchestrator from auto-defaulting to background.
...
- **Visual-Weight Probe (pre-delegation)**: Trigger-conditional — fires only when the sub-task is a user-requested HTML primary (1+ explicit format/share signal present, per the Exposure Determination HTML-request test above); whenever the deliverable is NOT a user-requested HTML primary the Probe is pass-through (agent-only / user-requested non-HTML modes — no HTML to style).
...
- **Claim-class scope of the provenance duty (FACTUAL, of which behavioural is one subset)**: the origin obligation in the **Provenance grammar** bullet above binds every FACTUAL claim — a count, a size, a duration, whether a file is present, any state the orchestrator read for itself — and not only the claims that happen to be about behaviour. So a behavioural claim additionally carries whatever label that grammar defines for it, while a non-behavioural factual claim still names its origin — the agent that produced it, or the instrument that measured it this turn. HONEST BACKING: honor-system, exactly as for the clause it scopes — no hook reads prose.
...
- **Premise audit (standing job, verdict-gating on BOTH verdicts) — gate operation here, duty text with the actors**: every cycle, both Stage-2 actors attack the delegation's premise register (grammar: `### Phase Notes` → Scan boundary and provenance) from the code.
  - Each actor reports every registered premise by handle, plus any behavioural claim in the delegation text absent from the register.
  ...
  - The orchestrator's part is composing the pair and supplying the register — it does not adjudicate the audit.
- **First-link question (standing job, verdict-gating on the DEV verdict, REVISION cycles only) — gate operation here, duty text with the actor**: when the plan entering this gate is a revision (a supersede chain root exists above it), the DEV participant additionally answers a standing first-link question inside its `feasible`/`infeasible` verdict, unasked.
  ...
  - The canonical duty body sits in the file its actor loads: DEV — `scoped/scope-dev.md` → `## Plan Direction Verification Gate [DEV+QA]`, first-link question — homed there for the same reason as the premise audit above (a duty homed in this file would reach neither actor).
...
`enforce-workflow-verify-stage.sh` does NOT infer verify roles from script layout (role information does not exist in code — any layout inference was a guess, since replaced); the AUTHOR declares them — mechanism parity with `[ENTRY-CLASS]` / `[SIZE-EST]` / `[DOC-ROUTE]` / plan-ref.
...
Rationale — and what "no hardcoding" does and does not forbid:
...
- So "no hardcoding" is the repo-default-and-automation rule, not an absolute ban on a live pin.
...
- **Subagent-side runtime complement**: the orchestrator-side splitting above stays the PRIMARY (honor-system) guard; the subagent also carries its own runtime budget meter + advisory — a SubagentStart TURN meter (maxTurns cap + 80% ceiling) and a PreToolUse TOOL_USE advisory at 70%/80% of a ~40 TOOL_USE budget.
  - The meter and the advisory make the truncation threshold observable + mid-run advised; the graceful `[COMPLETION]: needs_context` emit still stays behavioral (no mechanical brake).
...
- **`[SIZE-EST]` self-attestation token (sibling to `[ENTRY-CLASS]`)**: the orchestrator emits this token at EVERY DEV spawn.
  ...
  - **Scope of this contract — existence/self-attestation only**: the token records the orchestrator's own estimate; it does NOT mechanically verify the estimate's correctness (same existence-only boundary as `[ENTRY-CLASS]`, see `scope-dev.md` Spawn-time entry gate "Honest caveat").
  - Gate enforcement of this token's PRESENCE (never its correctness) runs on BOTH paths — manual via `enforce-verification-gate.sh` (guarded by `hook_is_subagent`, so it fires on orchestrator-origin spawns only) and ultracode via `enforce-workflow-verify-stage.sh` (DEV-gated, fires under `ENTRY_OK`, raw-scanning the script). Both BLOCK a DEV spawn missing the token, but they check PRESENCE only — never the estimate's correctness.
...
- **Decompose-analysis-by-domain FROM THE START (mandatory-upfront fan-out, NOT reactive split)**: when an audit/analysis/research target decomposes into N independent domains, compose it as N scoped agents AT DECISION TIME — each with its own read allowlist + condensed-return + a bounded analysis-mode `[SIZE-EST]` — plus ONE explicit reduce/synthesis phase (condensed returns, never raw transcripts).
  ...
  - This upfront decomposition makes the existing disjoint-file-ownership Automatic Parallelization partition MANDATORY-upfront for analysis work rather than reactive, sized per the effort-scaling table below and the `[SIZE-EST]` analysis-mode split trigger above.
...
Sizing each track still follows `[SIZE-EST]` + the effort-scaling shape above: partition by ownership FIRST, then size each track. The parallel fan-out is the DEFAULT, not a license to fragment — the over-fragmentation counter-caveat still applies (do not split one-budget work into 1-line tasks).

#### Workflow-runtime reconciliation (ultracode)

- Concurrency: the Workflow engine self-caps at a core-derived, per-machine runtime limit (the engine currently computes ~min(16, cores-2), machine-dependent — an INFORMATIONAL note of the engine's CURRENT formula, NOT a fixed number the orchestrator authors against) / ~1000 lifetime spawns. **The engine's runtime self-cap governs** concurrency under ultracode — the orchestrator authors the script WITHOUT any concurrency cap of its own and lets that core-derived runtime limit bound it; sequential waves are introduced ONLY when a fan-out would exceed the engine-enforced runtime limit.
