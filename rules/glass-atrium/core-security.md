...
- **Enforcement boundary (honest framing — primary control + a documented structural gap)**:
  - **PRIMARY enforcement = spawn-time frontmatter freeze (a real mechanical control, not just prose)**: the harness reads each agent's frontmatter `tools:` allowlist and FREEZES it at spawn time. A subagent simply cannot invoke a tool outside its frozen allowlist — this is the enforced LLM06 boundary, applied per-agent at spawn.
  - **Mid-task runtime per-agent allowlist check is structurally BOUNDED — not achievable at the current harness surface**: a PreToolUse hook firing on an inner subagent `tool_use` envelope cannot reliably identify WHICH agent is making the call. The inner envelope exposes only `agent_id` (an opaque token — `hook_is_subagent` merely keys on its presence; `track-outcome.sh` must recover the agent_type from a `.meta.json` sidecar), NOT the caller's `agent_type`. Contrast the spawn-time path, which DOES expose `tool_input.subagent_type` — so a per-agent decision is available at spawn but absent mid-task. Therefore full runtime per-agent tool-grant enforcement is NOT implemented and NOT claimed here.
  - **Optional future augmentation (advisory-only, NOT built)**: a spawn-time advisory pre-check could compare the delegation-requested tools against the target agent's frontmatter allowlist and warn on mismatch. This is heuristic — delegation prompts carry no structured tool-request field, so the requested-tool set must be inferred — making it advisory-only, weaker than the freeze, and flagged as optional future work rather than an enforcement layer.
