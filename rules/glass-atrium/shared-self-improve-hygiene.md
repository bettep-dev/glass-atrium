...
**Promotion record (executed 2026-07-27 for the 17-file enforced scope).** The condition above was confirmed met under the WIDENED terminator pattern (all four terminator-bearing alternatives share the `([^[:alnum:]_]|$)` class): the scope run reported, as of that date, `annotated=145 converted=9 unannotated=0 quality_reject=0` — the two census totals track whatever the scope currently holds and move with any later dedup (the shared sink extraction into `scripts/lib/pg-report-drop.sh` has since folded the duplicated sink and the six wiki reporting sites into one definition each), so only `unannotated=0 quality_reject=0` is a standing acceptance criterion — every closer-shaped site surfaced by the widening was block-scope adjudicated with zero false positives (six in-quotes hits are executed `$( )` command substitutions, none in inert literals or comments, none left unadjudicated), and the conversion set landed (the eight-site reporting-channel family plus the `autoagents-eval.sh` git-status conversion at named exit 5). The contract now in force:
