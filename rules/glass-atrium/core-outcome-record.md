...
- **Closing-turn discipline (prevention layer)**: emit the `[COMPLETION]` block as a DEDICATED FINAL assistant turn AFTER the last tool action — never end the run on a tool-result turn assuming the work is done. The SubagentStop transcript-synthesis net is the deterministic backstop; this discipline is the prevention complement.
