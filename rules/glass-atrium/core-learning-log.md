...
3. **No auto-persistence to user-facing memory**: a correction signal NEVER auto-writes `feedback_*.md` / `MEMORY.md` in the personal memory dir. The daemon's `directive_hint`→`feedback_*.md` auto-clustering path is gated-off by default in code (learning-aggregator.py, two-factor env + safety-marker authorization, fail-safe-to-OFF). Internal CTM/EPM accumulation (`memory/core-learning-log.md`) may still record the distilled pattern, but writing user-facing memory requires an explicit user instruction to remember — see Long-Term Memory Write-Gate condition 4.
