...
| **Decision** | Compose team + define scope | Decompose into sub-tasks · Consult capability hints (`domains` + descriptions) · Compose team + phase order · Justify by `domains`/description alignment · Define scope (files, change type, constraints) · **Probe each target path** (Read/Glob) before prompt assembly · Foreground Probe applies to harness-scope writes, Capability Probe applies to tool-grant validation, Compatibility Probe applies to agent runtime preconditions (see Phase Notes) · **Entry classification (DEV delegations)**: classify the task against the `scope-dev.md` Sprint Contract Gate → Sizable-task definition — sizable → author a plan first (enter the flow); judged simple/exempt → emit an `[ENTRY-CLASS] simple-task: <reason>` token in the delegation prompt (classify-always: the token is REQUIRED to silence the manual-path entry advisory) | Habitual delegation without rationale · Keyword/alias-based routing · Collapsing compound requests into single agent · Spawning subagent on unprobed paths · Spawning subagent whose compatibility preconditions are unmet | Team (`agents` + `reason` + `order`) + scope + constraints |
...
> **Ultracode note**: the Decision phase (routing + scoping) and these 4 serial Probes run BEFORE authoring a workflow script — the Workflow engine executes the mechanism but does NOT replace routing judgment or the probes. Encode their outcomes (selected agentType, scoped paths, probe verdicts) into the script; never let the engine's automation substitute for a probe. See `### Ultracode / Workflow-tool Mode`.
...
- **Backstop asymmetry (manual vs. ultracode) — name it honestly (canonical surface)**: the gate has two non-equivalent surfaces differing in KIND, not just strength. **Manual path**: `enforce-verification-gate.sh` (`PreToolUse(Agent)`) fires on every Agent spawn — a best-effort runtime advisory, weak (~17% same-batch-race caveat, see `### Ultracode / Workflow-tool Mode`), but a real hook. **Ultracode/Workflow**: `PreToolUse(Agent)` is BYPASSED; instead `enforce-workflow-verify-stage.sh` (`PreToolUse(Workflow)`, `settings.json`) statically scans the `script` and BLOCKS (exit 2) a DEV-spawning script lacking a `qa-code-reviewer` verify-stage token. This is a HEURISTIC, fail-open backstop — NOT full enforcement: it is comment-string-aware (a comment-only token fails it), ordering-aware (reviewer token BEFORE the first dev-* spawn), and applies a fail-open co-location heuristic (a dev-* DEV-verifier token within a character window of the reviewer token, catching "reviewer present but DEV hard-gate absent"); it does NOT validate DEV-verdict or gating-expression correctness, is fail-open on ambiguity / tooling drift, and is research-preview-fragile (re-verify on engine update). So the authoring-time obligation (honor-system self-discipline) REMAINS PRIMARY: encode a `{qa-code-reviewer, DEV}` verify-stage before any DEV implementation stage, gated on `pass`+`feasible`. The heuristic gate catches the gross omission (missing / comment-only / out-of-order reviewer token, or missing co-located DEV verifier); the subtle cases (incorrect DEV verdict, wrong gating expression) stay the author's responsibility, backed by the Red Flag self-check (`skills/glass-atrium-ops-orchestrator.md` → Red Flags "Missing-verify-stage guard"). Do NOT describe ultracode as "fully enforced" — it is heuristically backstopped + honor-system authored. Team composition, DEV hard-gate, complex-only scope, and max-1-revision are unchanged on both paths — only the backstop KIND differs. Canonical verify-stage skeleton (copy-verbatim SoT): `skills/glass-atrium-ops-orchestrator.md` → `### Pipeline Acceptance Criteria` → "In-script verify-stage".
...
- **Delegation-size discipline (per-delegation, distinct from MAX_CHILDREN concurrency)**: a single DEV delegation MUST be sized to finish within ONE agent budget — over-packing a single delegation is the truncation cause (sub-agent runs out of budget mid-work and emits no `[COMPLETION]`), and it is per-delegation SIZE, not the number of workflow stages.
  - **PRIMARY heuristic**: SPLIT when one delegation bundles >2 of {implement, write-tests, run-full-suite, report-consolidation} → sequential checkpointed sub-delegations (implement → orchestrator verifies cheaply → test → report).
  - **HARD SECONDARY trigger (anti-gaming, empirically anchored)**: if a single delegation is expected to exceed ~40 tool_uses (the measured 46-52 range that truncated a recent session), SPLIT regardless of bundle count — one implement bundle spanning 40+ files is still too big. This closes the hole a bundle-count-only rule leaves (one giant single-category implement bundle).
...
- **Orchestrator owns + AUTHORS INTO the script POLICY**:
  ...
  - **Quality gates as explicit verify-stages** — the 4 serial Probes (Decision phase), Plan Direction Verification (Stage-2 gate), Sprint Contract Gate, Pipeline Acceptance Criteria. The engine does NOT infer these — the orchestrator encodes them as gate stages in the script.
    - **Entry-class token (ultracode bridge)** — the Sprint Contract Gate's `[ENTRY-CLASS] simple-task: <reason>` token (Decision-phase-scoped to "the delegation prompt" on the MANUAL path) is instead recorded IN the workflow script under ultracode — recommended canonical home: a top-of-script `log()` string or `meta.description` field (greppability convention, NOT a comment prohibition — the gate raw-scans it, so any placement passes). The plan-ref token shares this placement. Canonical snippet: `skills/glass-atrium-ops-orchestrator.md` → Pipeline Acceptance Criteria "Entry-class token placement".
- **Hook layer split under the engine — `PreToolUse(Agent)` BYPASSED, `PreToolUse(Workflow)` static-scan heuristic gate backstops the in-script verify-stage (honor-system authoring obligation still PRIMARY)**: the engine's `agent()` spawns fire no `PreToolUse(Agent)` event (no `~/.claude/data/session-spawns/` trace), so `enforce-verification-gate.sh` is a manual-path-only safety-net, silently absent under ultracode. Consequence: a complex-plan workflow MUST encode the Plan Direction Verification (Stage-2 gate) as an explicit in-script `{qa-code-reviewer, DEV}` verify-stage sequenced BEFORE any DEV implementation `agent()`/`pipeline()` stage, gated on its `pass`+`feasible` verdict. The mechanical backstop, its heuristic/fail-open limits, and the PRIMARY honor-system authoring obligation are the SAME `enforce-workflow-verify-stage.sh` surface specified in `### Plan Direction Verification (Stage-2 gate)` → "Backstop asymmetry" (do NOT restate — it is heuristic-fail-open, honor-system-primary). Copyable `pipeline()` skeleton + Red Flag self-check: `skills/glass-atrium-ops-orchestrator.md` → `### Pipeline Acceptance Criteria` "In-script verify-stage" + Red Flags "Missing-verify-stage guard".
...
- Content: Goal + relevant file paths + key constraints + expected output format (4 items, current rule maintained) — plus the **Ripple radius** estimate per the canonical delegation-element list in the `glass-atrium-ops-orchestrator` skill (supplementary add-on; the 4-item core-payload framing is unchanged).
