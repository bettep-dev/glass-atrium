...
3. **section-order [warning]** — the canonical DESIGN.md section order (Visual Theme → Color → Typography → Spacing → Layout → Components → Motion → Voice → Anti-Patterns) is preserved. Unrecognized sections are kept, never flagged (Consumers MUST preserve unrecognized sections per glass-atrium-design-designer.md §section minimalism). Heading aliases bind the agent-body section names to their canonical slot — `UI Copy Rules` / `Copy Rules` → §8 Voice, `Do's/Don'ts` → §9 Anti-Patterns; alias tokens are multi-word or apostrophe-bearing on purpose, so no generic prefix (`do`, `copy`) can substring-match an unrelated heading.
