...
- **In-script verify-stage (ultracode — MANDATORY authoring obligation, honor-system PRIMARY, heuristically backstopped)** — *canonical skeleton; `orchestrator-role.md` cross-links here*: under ultracode the `enforce-verification-gate.sh` `PreToolUse(Agent)` hook does NOT fire for engine `agent()` spawns (`orchestrator-role.md` → `### Ultracode / Workflow-tool Mode`), but a `PreToolUse(Workflow)` static-scan heuristic gate now backstops the gross case: `enforce-workflow-verify-stage.sh` (wired in `settings.json`, firing-evidenced) statically scans the workflow `script` and BLOCKS (exit 2) a DEV-spawning script that lacks a `glass-atrium-qa-code-reviewer` verify-stage token. That gate enforces a hardened contract (a token mention is NOT enough on its own): (1) JS comments are STRIPPED string-aware before the scan, so a comment-only `glass-atrium-qa-code-reviewer` token does NOT satisfy the gate — it FAILS; (2) the reviewer's straight-quoted agentType token MUST appear BEFORE the first `dev-*` implementation spawn (ordering-aware); (3) a `dev-*` verifier MUST match the RECOGNIZED-SHAPE CLOSED SET (the DEV hard-gate half — stated here ONCE, every other mention in this skill points here): **two-tier token classification** — a dev/reviewer literal feeds impl-dev membership, verify-pair candidacy, and ordering ONLY in SPAWN position (agent-call first-arg or `agentType:` field value); a data config-field literal (e.g. `agent: 'glass-atrium-dev-*'` in a config array) is DATA and excluded — while the broad presence gates (Stage-2 exemption · zero-reviewer BLOCK_NOREV · entry/size-est) still see every literal, so parking literals in data fields never exempts a script; **co-location** — a spawn-position `dev-*` within ~2000 comment-stripped chars of ANY reviewer span; **parallel co-membership** — a spawn-position `dev-*` sharing a `parallel()` group with a reviewer (distance-independent); **pipeline stage-adjacency** — a reviewer-bearing `pipeline()` stage IMMEDIATELY followed by a stage holding a spawn-position `dev-*` OR a computed non-literal `agentType` (identifier / member / ternary) is the verify pair — the engine's canonical `pipeline(items, reviewerStage, devStage)` signature IS a recognized shape; recognition is confined to that adjacent stage, so a filler stage between the reviewer stage and the dev stage breaks the pair. Distances are measured on the COMMENT-STRIPPED script, so a long `//` or `/* */` comment placed between the reviewer and the impl DEV does NOT separate them (comments are removed before the distance is measured — do NOT pad with comments to fake co-location, and do NOT assume a comment gap pushes a real impl DEV out of range). RESIDUAL FALSE-POSITIVE SHAPES (doc-guided — the mechanism intentionally does not chase them): (a) a `pipeline()` whose stage arguments are function REFERENCES (not inline stages) hides the verify pair from the static scan → author the verify pair as the canonical INLINE two-stage `pipeline(items, reviewerStage, devStage)` signature (or an inline `parallel(qa, dev)` block); (b) a layout whose reviewer-to-dev gap exceeds ~2000 comment-stripped chars with no `parallel()` co-group and no adjacent pipeline pair → keep the dev spawn co-located with its reviewer (or co-group them in one `parallel()`). The gate is still preview-fragile and still does NOT validate DEV-verdict correctness (the `feasible` value does not exist at static-scan time), so it does NOT replace the authoring obligation — the orchestrator MUST still encode an explicit in-script verify-stage that PRECEDES the first DEV implementation stage and gates it on a combined `pass`+`feasible` verdict. "MANDATORY" binds the AUTHOR (you MUST write it); the heuristic gate stops gross violations (no reviewer, comment-only reviewer, reviewer-after-dev, reviewer with no recognized DEV verifier) but does not verify DEV-verdict correctness or gating-expression soundness. The authoring obligation + the Missing-verify-stage Red Flag self-check (`## Red Flags`) remain the PRIMARY discipline. Copyable shape (engine-agnostic vocabulary — `agent()`/`parallel()`/`pipeline()` are the Workflow primitives; do NOT hardcode preview-specific field names per the Non-brittleness caveat):
  ...
  // verifier sit co-grouped in the SAME parallel() verify block (a recognized verify-DEV shape —
  // closed set above), the reviewer precedes the LATER implementation dev-*, and every agentType is
  // a literal straight-quoted token (a computed agentType is recognized ONLY in the pipeline stage
  // immediately adjacent to a reviewer stage; anywhere else it fail-opens past the gate but defeats
...
**DEV-spawn 3-requirement pre-flight checklist (consolidated SoT — the SINGLE list the turn-0 `[WORKFLOW PRE-FLIGHT]` reminder and the Pre-submit self-check both point at, so no hand-maintained digest silently drops a requirement again)**: every DEV-spawning Workflow script MUST carry ALL THREE co-equal requirements before submission —
...
- **Missing-verify-stage guard (ultracode)**: a DEV-spawning workflow authored without a `{glass-atrium-qa-code-reviewer, DEV}` verify-stage preceding the first DEV implementation stage, gated on the combined `pass`+`feasible` verdict → **halt and re-author**. Honor-system self-check PRIMARY; the `enforce-workflow-verify-stage.sh` static-scan gate backstops gross omissions only. Hardened contract + skeleton (canonical): `### Pipeline Acceptance Criteria` → "In-script verify-stage".
  ...
  **Pre-submit self-check (run before submitting ANY DEV-spawning Workflow script — the gate's DEV-relevant block branches — entry gate + the `[SIZE-EST]` presence check (item 7 below) + three hardened verify conditions; the doc-routing leak is a separate gate branch with its own stderr — plus the verdict-gating the gate cannot see)**:
  ...
  3. a **`dev-*` verifier matches a recognized verify-DEV shape** — spawn-position two-tier classification · co-location · parallel co-membership · reviewer-adjacent pipeline stage (closed set stated ONCE at `### Pipeline Acceptance Criteria` → "In-script verify-stage"); a reviewer with NO recognized DEV verifier = DEV hard-gate absent → BLOCK (distances are measured AFTER comment-stripping, so do NOT pad with comments to fake co-location — and newlines inside `/* */` block comments SURVIVE stripping (line identity preserved), so comment padding still cannot shrink distances but does not merge lines);
...
- **Pre-verify Discovery `dev-*` order guard (ultracode)**: a `dev-*` used for Discovery/Design analysis positioned BEFORE the `glass-atrium-qa-code-reviewer` verify-spawn without matching a recognized verify-DEV shape (closed set: `### Pipeline Acceptance Criteria` → "In-script verify-stage") is classified as an un-gated implementation dev → `BLOCK_ORDER` (a legitimate Discovery/Design phase, NOT the implement stage). Fix: a NON-DEV Discovery agent (`glass-atrium-intel-researcher` / `glass-atrium-intel-planner` / `Explore`) OR a reviewer-first `{qa,dev}` Contract phase before any Discovery `dev-*`. Honor-system-primary framing unchanged; the exit-2 `BLOCK_ORDER` is the genuine mechanical block. Skeleton: `### Pipeline Acceptance Criteria` → In-script verify-stage 3-phase variant.
