...
**Workflow pre-flight (run before EVERY Workflow call)**: 1.

1. entry-classify (sizable digest: `orchestrator-role.md` Delegation Workflow Decision row) — sizable → plan-ref · simple → `log('[ENTRY-CLASS] simple-task: <reason>')` / `meta.description` (clears ONLY the entry gate; canonical snippet: this file → "Entry-class token placement"); 2.
2. `[SIZE-EST]` self-attestation — at EVERY DEV spawn emit the sibling `[SIZE-EST] bundles=N tool_uses~=N — <reason>` token via the same `log()` / `meta.description` home as the entry token (format + honesty/existence-only framing = SoT `orchestrator-role.md` → `### Spawn Budget` `[SIZE-EST]` bullet, do not restate — under-estimate = DANGEROUS error, round UP on borderline; PRESENCE-only gate, never correctness), a CO-EQUAL requirement with the entry token, the verify-stage, and the declaration block, NOT a sub-clause of entry-classify (consolidated list: this file → "DEV-spawn 4-requirement pre-flight checklist"); 3.
3. DEV spawn → {glass-atrium-qa-code-reviewer, dev-*} verify-stage BEFORE the first dev-* (skeleton: this file → Pipeline Acceptance Criteria · self-check: this file → Red Flags); 4.
