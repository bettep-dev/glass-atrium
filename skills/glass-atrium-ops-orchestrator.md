...
- **Compact-schema authoring (prevents the invalid-emission mode by construction)**: keep the StructuredOutput payload small enough to actually validate —
  ...
  - **(b) Cap EVERY field — but size each cap GENEROUSLY to the field's realistic worst-case, NEVER an aspirational small number**: a cap set TOO TIGHT reproduces the identical summary-collapse loop as an uncapped field (both fail the SAME way — see the LENGTH root-cause above). Concrete FLOORS to size against: a free-text `summary`/`analysis` field → generous, ≥ ~1500 chars (or UNCAPPED with the file-handoff in (a)); a per-row / per-item string that carries evidence → ≥ ~300-400 chars; an array `maxItems` → the TRUE expected max item count, not a hopeful small number. An uncapped field invites the oversized-then-collapse loop; a too-tight cap forces the collapse outright — aim for the realistic-worst-case middle.
  ...
  - Proven by the v2 rewrite of the schema-failure workflow — AND by THIS session's over-tight-cap collapse loop, where too-small `maxLength` caps (`260`/`160`/`900`/`400`) reproduced the exact retry-cap-exceeded failure on fields that were capped but not generously.
