...
- entry-classify (sizable digest: `orchestrator-role.md` Delegation Workflow Decision row) — sizable → plan-ref
  - simple → `log('[ENTRY-CLASS] simple-task: <reason>')` / `meta.description` (clears ONLY the entry gate; canonical snippet: this file → "Entry-class token placement"); 2.
- `[SIZE-EST]` self-attestation — at EVERY DEV spawn emit the sibling `[SIZE-EST] bundles=N tool_uses~=N — <reason>` token via the same `log()` / `meta.description` home as the entry token (format + honesty/existence-only framing = SoT `orchestrator-role.md` → `### Spawn Budget` `[SIZE-EST]` bullet, do not restate — under-estimate = DANGEROUS error, round UP on borderline; PRESENCE-only gate, never correctness), a CO-EQUAL requirement with the entry token, the verify-stage, and the declaration block, NOT a sub-clause of entry-classify (consolidated list: this file → "DEV-spawn 4-requirement pre-flight checklist"); 3.
- DEV spawn → {glass-atrium-qa-code-reviewer, dev-*} verify-stage BEFORE the first dev-* (skeleton: this file → Pipeline Acceptance Criteria
  - self-check: this file → Red Flags); 4.
- `[AGENT-COMPOSITION]` declaration — every DEV-spawning script carries exactly ONE declaration block in a `/* */` comment (grammar + verdict set = SoT `orchestrator-role.md` → `### Plan Direction Verification (Stage-2 gate)` declaration-contract bullet, do not restate — absence → `block-nodecl`
  - malformed → `block-grammar`, both exit 2), a CO-EQUAL requirement with the other three (consolidated list + declaration-bearing skeletons: this file → "DEV-spawn 4-requirement pre-flight checklist" + Pipeline Acceptance Criteria); 5.
- encode Decision outcomes into the script — routed agentType per spawn, scoped target paths, the 4 probe verdicts (probes run BEFORE authoring; the engine executes but never substitutes for a probe); 6.
- typed agentType on every spawn; 7.
- author within the engine's runtime self-cap — no fixed concurrency number of the orchestrator's own (the core-derived, per-machine engine limit bounds concurrency); 8.
...
- user explicitly requested a local destination → stamp the `[DOC-ROUTE] user-requested-local:` token in the script (NEVER stamped without an actual explicit user request; canonical snippet + self-check: this file → "[DOC-ROUTE] token placement" + Red Flags); 10.
- PREVIEW before submit — run `enforce-workflow-verify-stage.sh --lint <file>` (offline: reads the RAW script, reuses the IDENTICAL verdict dispatch, so `exit 0` = will pass the gate and `exit 2` prints the block reason — the same-code-path preview, never a drift-prone reimplementation) as the FINAL self-check; `--lint --template` prints the canonical [AGENT-COMPOSITION]/entry/[SIZE-EST] scaffold.
