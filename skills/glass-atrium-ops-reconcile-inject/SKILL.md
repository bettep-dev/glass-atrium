...
Three rosters are UNTRACKED by design, for two DIFFERENT reasons. `BUDGET_ANALYSIS_AGENTS` and `WIKI_UNTRUSTED_AGENTS` are governance memberships that are not roster-derivable, so a predicate would be a second copy of the array rather than a check on it. `PLAN_GATE_AGENTS` IS roster-derivable — it is the whole DEV roster, the same shape `MINIMALISM_AGENTS` carries — and is untracked for the other reason: it is a declaration deliberately SEPARATE from `MINIMALISM_AGENTS`, so narrowing one cannot silently narrow the other, and a newly registered DEV agent is added to it by hand. A reconcile run leaves each of the three byte-identical.
