...
description: On-demand audit-and-fix for the Atrium architecture diagrams (diagrams-source.ts) against the live filesystem. Runs the deterministic computeArchDrift() count, adds LLM semantic-completeness checks the machine cannot do (orphan nodes, prose-vs-behavior mismatch, new agent missing from the team graph, broken cross-refs), and emits an actionable fix report; then, on a deliberate human invocation, immediately APPLIES the fixes via a foreground dev-node delegation (count writes to arch-invariants.ts, semantic edits to diagrams-source.ts, build-gated launchctl restart, re-verify). Use this before a release, when the monitor's "최신화 필요" drift badge appears, or after adding/removing an agent/hook/rule/launchd job/skill. Pass --dry-run to report only, applying nothing. Excludes runtime daemon-health checks; apply happens only on human invocation, only via foreground dev-node delegation, never silently or from a daemon.
---
...
**Apply boundary**: the main session is Write/Edit-blocked on `~/.glass-atrium/` (enforce-delegation + Harness Path Protection), so Stage 4 never edits in-session — it delegates every edit, the build, and the restart to a **foreground** `dev-node` agent. The one-command UX is preserved; the edit is internal to the delegation.
...
Stage 4 turns the Stage 3 report into edits, a build, and a restart, then re-verifies. The main session cannot write under `~/.glass-atrium/` (enforce-delegation + Harness Path Protection), so all edits/build/restart are delegated to a **foreground** `dev-node` agent — never edited in-session, never backgrounded, never daemon-triggered. Run the steps in this fixed order:
...
**4. Assemble the dev-node delegation package.** The main session composes (does not execute) the edit spec:
...
**5. Foreground delegation → `dev-node`** (`~/.glass-atrium/` is harness-adjacent → background FORBIDDEN). The agent performs, in order:
...
`diagrams-source.ts` and `arch-invariants.ts` forbid in-file changelogs — current-state invariants only. The Stage 4 apply (and any other follow-up edit) MUST NOT add Wave/CID/date history to either file (the file header states this); pass this constraint verbatim into the dev-node delegation package. The fix report records "what to change"; the "why" goes in the git commit message, the history is `git log`. Never instruct an editor to append a changelog comment to these files.

## Prohibitions

- **Edits only in Stage 4, only on deliberate human invocation, only via foreground dev-node** — the main session never edits `diagrams-source.ts` / `arch-invariants.ts` in-session (Write/Edit blocked by enforce-delegation + Harness Path Protection). An edit happens ONLY in Stage 4, ONLY when a human ran `/glass-atrium-ops-verify-arch` without `--dry-run`, ONLY through a foreground `dev-node` delegation. NEVER silently, NEVER from a daemon, NEVER backgrounded. Under `--dry-run` no edit occurs at all.
...
- The main session edited a SoT file in-session → Stage 4 edits MUST go through a foreground `dev-node` delegation.
...
- [ ] **Foreground delegation**: Stage 4 edits/build/restart ran via a foreground `dev-node` agent — never edited in-session, never backgrounded, never daemon-triggered.
