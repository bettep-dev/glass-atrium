...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  112 tracked `*.py` files plus the tracked root `requirements.txt` and the CI-only
...
All other imports across the 112 tracked `*.py` files are Python standard library or
