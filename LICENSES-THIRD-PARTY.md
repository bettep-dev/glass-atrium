...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  104 tracked `*.py` files plus the tracked root `requirements.txt` and the CI-only
...
All other imports across the 104 tracked `*.py` files are Python standard library or
