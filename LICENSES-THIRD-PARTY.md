...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  80 tracked `*.py` files (no `requirements.txt` / `pyproject.toml` exists in the repo).
...
All other imports across the 80 tracked `*.py` files are Python standard library or
