...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  110 tracked `*.py` files plus the tracked root `requirements.txt` and the CI-only
...
All other imports across the 110 tracked `*.py` files are Python standard library or
