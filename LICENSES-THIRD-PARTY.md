...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  106 tracked `*.py` files plus the tracked root `requirements.txt` and the CI-only
...
- `psycopg` (v3) — 11 production files: `hooks/_pg_dual_write.py`,
...
All other imports across the 106 tracked `*.py` files are Python standard library or
