...
- **Manifests audited**: `monitor/package.json` (versions pinned by the tracked
  ...
  89 tracked `*.py` files plus the tracked root `requirements.txt` (no `pyproject.toml`
  exists in the repo).
...
All other imports across the 89 tracked `*.py` files are Python standard library or
