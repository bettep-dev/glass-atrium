...
- **Sensitivity self-check before POST (every exposed HTML primary — pointer)**: before the single POST of every exposed HTML plan (either HTML request test signal), record `sensitivity_scan: clear | N items (category §locator, …)` in turn-0 — count + category + locator only, never quoting flagged content; finding → no POST without user confirmation; zero findings = silent pass. Canonical rule text: `scope-report.md` Output Format Routing → Emission contract → Sensitivity self-check before POST.
...
- PRIMARY — Mermaid type mapping (14 permitted types → information shape · see `glass-atrium-intel-planner.md` "Abstraction Level & Diagram Requirement Matrix") · section composition (3-layer Pyramid rhythm)
