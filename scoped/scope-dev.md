...
**Delivered mirror: NONE — and the route out is the injector, not a body mirror.** Measured 2026-09-10: no `agents/glass-atrium-dev-*.md` body carries any text from this section — not the hard-gate rule, not the load-bearing premise check, not the first-link question, not the non-waiver clause — and this file reaches none of those thirteen agents at spawn, so every DEV-side duty stated here reaches no actor. The residual those actors need is now marker-wrapped at the END of this section, but nothing extracts it yet, so delivery is still nothing: that block is inert until the injector is wired to it. Two measurements picked that route over thirteen body mirrors. (a) The assembled worst-case DEV context has room for a block that size — the arithmetic that appeared to forbid it subtracts a deliberately tight PLAN bound from the ceiling, and a plan bound is not a measured assembly, so the apparent headroom was far smaller than the real one. (b) Thirteen bodies would become thirteen unpinned copies of a sentence one suite reads byte-for-byte out of THIS file, which is the same drift this section exists downstream of. Gate operation is annotated on its own side at `rules/glass-atrium/orchestrator-role.md` → `### Plan Direction Verification (Stage-2 gate)`, and the reviewer half at `scoped/scope-qa.md` → `## Plan Direction Verification Gate [DEV+QA]`, which records the same undelivered status for its own actor. Do not treat this section as delivered, and do not home a further duty here alone.
...
<!-- BYTE-BUDGET: authored for inject-scope-rules.sh but NOT WIRED — no extract call names this pair, so it is inert today and the assembled DEV context is byte-unchanged. Wiring = a third extract call sourced from this same file against the same thirteen-member DEV roster the minimalism block uses, given a drop-priority slot ABOVE the four proven blocks so ceiling pressure never sheds naming. Content contract <=663B, from a bisect of the real DEV assembly (measured 2026-09-10: nothing sheds at 663, both naming and budget-dev shed at 665) — re-derive it rather than trusting this line, and do NOT derive it instead by subtracting the nodrop suite's pinned dev-front bound from the ceiling, which yields a far smaller figure because that bound is a deliberately tight PLAN bound, not the assembly. Current content is ~587B by hand count, so verify with an actual byte count when wiring, and re-run hooks/test/inject-scope-rules-nodrop.bats on any rewording. -->
