...
- **Pattern recognition** (Search → Read → Mirror):
  ...
  - **Step 3 — Mirror**: apply the extracted CODE-FORM axes — `naming case` / `import order` / `error+log pattern` / layout.
    - Identifier FORM is not a mirrored axis. The canonical verb set and the boolean / stative forms come from the naming canon (`glass-atrium-dev-naming`), and a sibling whose identifiers diverge from it is NOT a precedent — the sibling settles case and separator convention only.
