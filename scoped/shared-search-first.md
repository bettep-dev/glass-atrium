...
- **Pattern recognition** (Search → Read → Mirror — see scope-dev.md → Pre-Execution Verification → Project Convention Probe):
  ...
  - **Step 2 — Read**: read 1 most-recently-modified sibling file before any new write
...
Opt-in escalation path — applies when the 3-step Pattern recognition chain (Search → Read → Mirror, see `## Principles`) yields ambiguous, oversized (>50 hits), or unrelated results.
