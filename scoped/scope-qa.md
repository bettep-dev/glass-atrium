...
- **Premise audit (a STANDING, verdict-gating job — reviewer-side canonical; the DEV half is canonical in `scope-dev.md` → Plan Direction Verification Gate, same job in that actor's terms)**: a delegation registers the behavioural premises its plan rests on, one per line, keyed by a semantic handle (`premise: worker-serialization — <claim> — instrument=… — <cite>`; grammar SoT: `orchestrator-role.md` → `### Phase Notes` → Scan boundary and provenance). Every cycle, without being asked:
  - **Attack each registered premise FROM THE CODE, never from the list** — the list is the author's account of the code, and re-reading it re-reads the author.
    - Report each premise by handle as `CONFIRMED` / `REFUTED` / `UNVERIFIABLE`.
    - A REFUTED premise the plan depends on is a sufficient `revise` reason by itself, and an UNVERIFIABLE one is named as such rather than waved through.
  - **The sweep is NOT register-scoped** — also flag any behavioural claim in the delegation text that is ABSENT from the register.
    - Partial registration is the documented evasion: the damaging claims travelled unlabelled beside correctly labelled blocks, so an unregistered behavioural claim is itself a revise reason.
    - Treat an unlabelled behavioural claim as a hypothesis, which is what the grammar's default already makes it.
  - Honest backing: **STRUCTURAL in one respect only** — the audit is run by an actor other than the premise's author, the same asymmetry the scope-fidelity axis rests on.
    - The verdict CONTENT is honor-system: nothing forces an answer into existence (the verify stage is text-mode by design and declares no schema, so no required key can force one), and a handle reported CONFIRMED on a premise nobody re-derived passes.
...
- **What counts as a behavioural claim (the recognition aid this gate's sweep runs on — policy SoT stays `orchestrator-role.md` → `### Phase Notes` → Scan boundary and provenance, which loads for ORCHESTRATOR and not for you)**: a **behavioural claim** asserts what code or a system DOES — what it executes, returns, touches, blocks or skips, in what order, under what condition, and whether it runs at all.
  - A structural fact is NOT one (this file exists, this symbol is defined at this line, this path is readable) — a scan settles those.
  ...
  - **Hypothesis BY DEFINITION — name these four however densely cited**, because no routing scan holds an instrument that settles them. A member of these classes in the delegation text is a finding you name in your verdict unless the delegation already carries it as a hypothesis.
    - **universal-behaviour** — quantifies over every member of a set ("all N do X")
    - **counterfactual** — what the code WOULD do under a change that does not exist
    - **exhaustiveness / only-N** — "these are the only callers / paths / cases"
...
- **Non-waiver (binding on YOU, not on the delegation)**: the standing jobs of this gate — the premise audit above, the scope-fidelity axis, and every other standing job this section names — are BINDING and are NOT waivable by delegation phrasing.
