...
- **Open, owner elsewhere — the `###` form of the `Resilient Workflow Authoring` citation outside the charter**: the charter cites `skills/glass-atrium-ops-orchestrator.md` → `#### Resilient Workflow Authoring [ORCHESTRATOR]`, the heading's full literal.
  - The `###` form appears in `agents/glass-atrium-meta-prompt-engineer.md` (the EDITABLE line "binding rules that live ONCE"), in three skeleton code comments in `skills/glass-atrium-ops-orchestrator.md`, and in the resilience ADVISORY string of `hooks/enforce-workflow-verify-stage.sh`.
  - Not taken here: this note owns none of those files. The skill's prose citations of `#### Resilient Workflow Authoring` already carry the heading's level.
