...
Companion to `scoped/scope-qa.md`, which now carries QA-agent-facing duties only. Everything here addresses a maintainer, the orchestrator or a corpus editor — no running QA agent is obliged by any of it. Read it before editing that file; do not inject it.
...
- Membership is declared on the registry rows (`agent-registry.json` → each agent's `rules.scope`) and in `rules/glass-atrium/core-compliance-matrix.md`. The rule file itself no longer restates it, and its Tier-2 loading stanza was dropped with the rest of the delivery bookkeeping.
- QA additionally loads `scoped/shared-comment-logging.md` always and `scoped/shared-hook-capability-contract.md` when reviewing hook work — declared in the matrix, not in the rule file.

## Which agent the file binds

- Every duty in the rule file binds `glass-atrium-qa-code-reviewer`. None binds `glass-atrium-qa-debugger` — verified passage by passage in the audit below, and stated in the rule file's opening line because it is the single most useful fact the debugger can learn from the file.
- 71% of the surviving duty text is the Stage-2 gate, which fires only on a `{glass-atrium-qa-code-reviewer, DEV}` plan-verification spawn. That condition is now stated at the section itself rather than left implicit.

## Retention ledger — who reads each section

Every section kept in the rule file is kept because a NAMED reader outside the file resolves to it. A section nothing in this table points at should be deleted rather than kept for completeness.
...
| `## D8 Visual Decision Sub-Pass (HTML Primary Deliverables) [QA]` | glass-atrium-qa-code-reviewer scoring an HTML primary; the designer skill's veto line | `agents/glass-atrium-qa-code-reviewer.md` · `skills/glass-atrium-design-html-co-emission/SKILL.md` · `scoped/scope-report.md` · `agents/glass-atrium-intel-planner.md` |
...
- **Heading stability — named verbatim from outside `scoped/scope-qa.md`**: every `##` heading in the table above, plus exactly one `###` — `### Reviewer verdict`, cited by `rules/glass-atrium/orchestrator-role.md` → `#### Team composition and verdicts`. Renaming one of these dangles a live citation.
...
- The Regression Risk "Undelivered, and a RELOCATE candidate for the reviewer body" note — superseded by the routing row under **How the reviewer reaches this file** below.
...
No hook injects the rule file, so the only channel is an absolute-path Read the reviewer's own delivered body orders. Two routes exist, and each names the section it lands on:

| Route | Lands on | Where the pointer lives |
|---|---|---|
...
| every code review, before the `Regression Risk` label is assigned | `## Regression Risk Estimation [QA]` | `agents/glass-atrium-qa-code-reviewer.md` → `#### Template field notes` → **Regression Risk** |

- **Pointers, not mirrors**: both sections stay single-sited here, so the reviewer pays one Read per route and no copy can drift. Mirroring either into the body would trade this dangler for a drift pair needing a co-edit row.
- **The Stage-2 route also scopes the verdict vocabulary**: the body states that `pass` / `revise` is this gate's and `Pass / Conditional Pass / Reject` is its own template's — the same closure the rule file makes from its side.
- `hooks/test/inject-scope-rules-nodrop.bats` withholds the plan-gate block from the reviewer on the ground that it "reads its own duty from scope-qa.md". The roster decision was always right on its own grounds, and the stated ground now holds.
...
- **Nothing reads its body.** It carries no marker block, no extracted literal and no pinned needle — the corpus-wide `AGENT-INJECT` machinery does not touch it.
- The couplings are all on the path spelling and the file's existence, never on a sentence inside it:
...
- The manifest hash for `scoped/scope-qa.md` and the row for this companion both come from `scripts/generate-manifest.sh`, a whole-tree regeneration barrier — so an edit to either lands in the same commit unit as that regeneration, never ahead of it.
