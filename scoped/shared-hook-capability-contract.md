...
- **Fail-open default**: lifecycle hooks that cannot block (`SessionStart`, `SubagentStart`, `Stop`/`SubagentStop`) MUST `exit 0` on internal error — never break a session on hook failure (`GLOBAL_RULES.md` Hook Operation Policy). Exception: `~/.claude/autoagent/` self-improvement pipeline hooks follow the loud-fail principle (`shared-self-improve-hygiene.md`).
