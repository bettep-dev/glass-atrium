#!/usr/bin/env python3
"""Shared compliance-rate SoT (plan doc 57031 T3/T4).

Single home for the compliance-rate computation that the daemon cycle
(autoagent/daemon_cycle.py) and the learning aggregator
(hooks/learning-aggregator.py) previously duplicated:

  ``compute_compliance_rate`` — a TRUE compliance rate derived from an ACTUAL
...
The JSONL signal-store writer half (``append_signal`` + ``SIGNAL_STORE_FILE``)
is RETIRED: both populated families now land in Postgres and the store is a
frozen archive, so the module keeps only the rate half.

DETECTION-ONLY: nothing here blocks or raises into a cycle. Every public helper
is fail-soft — an unreadable log degrades to insufficient-data, never an
