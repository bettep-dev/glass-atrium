#!/usr/bin/env bash
# inject-scope-part-05.sh — the SubagentStart slot that carries part 05 of the split
...
if [[ ! -r "${_ga_chunk_lib}" ]]; then
  _ga_slot_fail "library unreadable: ${_ga_chunk_lib}; slot skipped (part=05)"
  exit 0
fi
# shellcheck source-path=SCRIPTDIR source=lib/inject-chunk.sh
source "${_ga_chunk_lib}" || {
  _ga_slot_fail "library source failed: ${_ga_chunk_lib}; slot skipped (part=05)"
  exit 0
}
