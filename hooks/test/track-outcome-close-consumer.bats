...
# (CI) aborts on it — measured, bats 1.13.0 on both legs, so bash is the variable, not bats. oc/no
# echo a diagnostic + return non-zero so each caller's `|| return 1` aborts the test AT the failing
# assertion on both.
