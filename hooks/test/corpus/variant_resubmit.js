...
impl-computed: glass-atrium-dev-shell, glass-atrium-dev-node
[/AGENT-COMPOSITION] */
export const meta = {
...
const BATCHES = [
  { id: 'H1', lang: 'shell', files: 'hooks/enforce-workflow-verify-stage.sh (single file, 431 comment lines — the biggest hook). SOFTER ceiling: this is the workflow verify-stage gate; preserve every gate-semantics rationale (BLOCK_ORDER positional rule, entry-token raw-scan, DEV-classification heuristics, fail-open enumerated-case defaults, SIZE-EST/ENTRY-CLASS/DOC-ROUTE token contracts) as essence lines.' },
  { id: 'H2', lang: 'shell', files: 'hooks/track-outcome.sh (single file, 345 comment lines). SOFTER ceiling: the outcome recorder; preserve the parse-tier contract (the COMPLETION-tag newline anchor, multi-line-form mandate, reverse-scan preferring the last COMPLETION-bearing text), grader input-surface invariant (block-resident-only evidence), synthesis/downgrade_origin semantics, style_ref cross-verification, correction-signal keying.' },
  { id: 'H3', lang: 'shell', files: 'hooks/inject-scope-rules.sh, hooks/cost-tracker.sh, hooks/detect-secret-file-write.sh, hooks/block-doc-routing-leak.sh. SOFTER ceiling (security-dense): preserve the LLM01 path-traversal note, the AGENT-INJECT extract_block allowlist rationale, secret-pattern coverage + DOCUMENTED RESIDUAL boundary, doc-route stamp path-scoping rules.' },
  { id: 'H4', lang: 'shell', files: 'hooks/hook-utils.sh, hooks/validate-scope-drift.sh, hooks/lint-workflow-template-literal.sh, hooks/telemetry-activation.sh, hooks/post-edit-typecheck.sh, hooks/post-edit-outcome-sync.sh, hooks/pre-compact.sh, hooks/inject-session-context.sh (8 smaller hooks). Standard ceiling; preserve the hook-utils WHY notes (LLM01), PLAN_FILE/scope-drift contract, template-literal bash-pitfall rationale, CONTINUITY header contract.' },
  { id: 'S1', lang: 'shell', files: 'run: ls scripts/*.sh | sort | sed -n "1,10p" — those 10 files exactly. Do NOT touch scripts/lib or scripts/test or scripts/agent_lifecycle.' },
  { id: 'S2', lang: 'shell', files: 'run: ls scripts/*.sh | sort | sed -n "11,20p" — those 10 files exactly. Do NOT touch scripts/lib or scripts/test or scripts/agent_lifecycle. NOTE: if update.sh is in your slice it is SOFTER ceiling (5 SECURITY annotations + apply-lock/update-pause contracts).' },
  { id: 'S3', lang: 'shell', files: 'run: ls scripts/*.sh | sort | sed -n "21,29p" — those 9 files exactly. Do NOT touch scripts/lib or scripts/test or scripts/agent_lifecycle. NOTE: if update.sh is in your slice it is SOFTER ceiling (5 SECURITY annotations + apply-lock/update-pause contracts); monitor-log-rotate.sh must keep the launchd fd-preservation why (launchd never reopens fds -> truncate not unlink).' },
  { id: 'SL', lang: 'shell', files: 'all scripts/lib/*.sh (11 files). SOFTER ceiling for apply-lock.sh (3 SECURITY; pid-liveness + TTL stale-reclaim contract), apply-gate.sh, sensitive-refusal.sh, update-pause-flag.sh; keep the mirror-farm + daemon-bootstrap-common ordering/why notes.' },
  { id: 'M1', lang: 'ts', files: 'all monitor/src/server/types/*.ts AND all monitor/src/server/clauded-docs/*.ts (TypeScript). EXCEPTION: leave monitor/src/server/types/architecture.ts untouched. Preserve: the control-chars lastIndex-on-global-regex gotcha (a real bug-class warning), RFC 8259 ref, html-validator policy notes, the wiki types local-enum duplication rationale. Self-evident field-name comments are the main cut target in types/.' },
  { id: 'M2', lang: 'ts', files: 'all monitor/src/server/routes/*.ts (TypeScript; biggest absolute volume: clauded-docs.ts ~791 comment lines, improvement.ts ~545). EXCEPTION: leave monitor/src/server/routes/architecture.ts untouched. Ratio is already moderate — cut verbose block narration + duplicated route-flow prose; PRESERVE route-contract/edge-case notes (optimistic-lock expected_hash flow, status-only cascade, supersede semantics, doc_status transitions).' },
  { id: 'M3', lang: 'ts', files: 'every remaining monitor/src/server/**/*.ts EXCLUDING the types/, clauded-docs/, routes/ directories AND EXCLUDING everything under monitor/src/server/architecture/ (leave completely untouched — already lean). Also do NOT touch monitor/src/generated.' },
  { id: 'T', lang: 'shell', files: 'build-glass-atrium.sh, install.sh, monitor/scripts/oss-db-setup.sh. install.sh is user-facing bootstrap — keep its safety/why notes; oss-db-setup.sh keep any destructive-guard / idempotency rationale.' },
  { id: 'TI1', lang: 'bats', files: 'all hooks/test/*.bats (14 files). TEST-INTENT TIER: NO flat ratio target — remove ONLY banners + restated-assertion prose; "what this asserts / why this test exists" intent notes are near-load-bearing, keep them (one-essence-line where verbose). GATE: run bats on each edited file — must pass with the same test count.' },
  { id: 'TI2', lang: 'bats', files: 'all scripts/test/*.bats (17 files). TEST-INTENT TIER: same rules as hooks/test — banners + restated-assertion prose only; keep intent notes; run bats per edited file, same count, all pass.' },
  { id: 'SPA', lang: 'shell', files: 'glass-atrium (the thin loader, ~908 lines) + lib/ga-tui-caps.sh + lib/ga-tui-term.sh + lib/ga-tui-primitives.sh + lib/ga-tui-frame.sh. NOT in the baseline -> snapshot-first floor. PRESERVE: the loader R8 contract notes (strict-mode/trap loader-owned, siblings never re-arm), source-order/readonly MIN_ROWS-after-MENU_COUNT rationale, set-u stub grouping notes, cleanup/fd-4 lifecycle notes, ERR-trap LINENO note. AFTER editing run this targeted bats subset and ALL must pass: test/workbox-bar-count-visible-during-resolve.bats test/continuous-index.bats test/install-step-counter-grand-total.bats test/spinner-cursor-visibility.bats test/spinner-jobcontrol-restore.bats.' },
  { id: 'SPB', lang: 'shell', files: 'lib/ga-tui-workbox.sh + lib/ga-tui-run.sh + lib/ga-tui-monitor.sh + lib/ga-tui-dispatch.sh + lib/ga-tui-preflight.sh. NOT in the baseline -> snapshot-first floor. SOFTER ceiling: these carry the A1 blank-box fix + counter contracts. PRESERVE as essence lines: the RC-1 render contract comments (COMPOUND gate run+bar-present; publish STEP_BAR_CUR BEFORE the spinner fork; seed STEP_INDEX only-when-empty), the R3 no-subshell brace-group counter contract (run_step/run_gate_quiet), GRAND_TOTAL freeze + disp_n fallback rationale, GA_TUI_STEP exit_step/die_step dispatch notes, monitor stop/restore ownership guards, preflight fd-4 + PREFLIGHT_EXIT_BLOCKED + bounded-poll/anti-hang notes. AFTER editing run this targeted bats subset and ALL must pass: test/workbox-bar-count-visible-during-resolve.bats test/continuous-index.bats test/install-step-counter-grand-total.bats test/run-step-fail-return.bats test/deps-preflight-noninteractive.bats test/pg-detect-connect-deadline.bats test/uninstall-detached-daemons.bats test/monitor-build-selfheal.bats.' },
]

function reviewerPrompt(b) {
  return `Pre-compression review (READ-ONLY) for one comment-reduction batch. ${COMMON}
...
phase('Reduce')
// Verify-team co-location: qa-code-reviewer (stage 1) gates each batch; the dev verifier/implementer literals sit in stage 2 below.
const results = await pipeline(
  BATCHES,
  (b) => agent(reviewerPrompt(b), { label: 'preserve:' + b.id, phase: 'Reduce', agentType: 'glass-atrium-qa-code-reviewer', model: 'opus' }),
  (review, b) => agent(devPrompt(b, review || '(no reviewer output)'), { label: 'reduce:' + b.id, phase: 'Reduce', agentType: (b.lang === 'ts' ? 'glass-atrium-dev-node' : 'glass-atrium-dev-shell'), model: 'opus' }).then(r => ({ batch: b.id, impl: r ? 'done' : 'EMPTY' })),
)
