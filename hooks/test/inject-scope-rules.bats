...
#   scope blocks: AGENT-INJECT:NAMING (DEV(12, excluding dev-swift) + qa-code-reviewer) and the two
#   BUDGET blocks (BUDGET-DEV → DEV(9, minus the four daemon carriers) · BUDGET-ANALYSIS → 6 curated
#   analysis consumers), plus the ceiling drop order across all six droppable blocks.
#
...
#   is agent-agnostic and would otherwise add noise to every run). The comment-logging / style_ref /
#   minimalism sources are pointed at /nonexistent by DEFAULT so a "naming present" assertion proves
#   the naming block specifically, not an accidental match in a sibling block. The naming AND budget
...
setup() {
  ...
  # Same rationale as the naming fixture: the HOME-anchored real file is absent in CI.
...
# Drive the hook with a SubagentStart envelope wrapping $1 (agent_type). The comment/style_ref
# scope sources + AGENTS_DIR are sandboxed to /nonexistent and the meter is off, isolating the
# naming + budget blocks. The naming and budget sources default to the hermetic fixtures built in
# setup() (so the positive assertions do not depend on the HOME-anchored real files, absent in CI);
# $2 (optional) overrides INJECT_SCOPE_RULES_NAMING_SRC and $3 (optional) overrides
# INJECT_SCOPE_RULES_BUDGET_SRC for the fail-open / absent-marker tests.
run_hook() {
  local agent="${1}" naming_src="${2:-${NAMING_FIXTURE}}" budget_src="${3:-${BUDGET_FIXTURE}}"
  run bash -c '
    agent="$1"; hook="$2"; naming_src="$3"; budget_src="$4"
    ...
    printf "%s" "${payload}" | env \
      ...
      bash "${hook}"
  ' _ "${agent}" "${HOOK_SH}" "${naming_src}" "${budget_src}"
}
...
# (c2) BUDGET rosters — BUDGET-DEV goes to DEV(9) minus the four daemon carriers (dev-nestjs,
...
@test "dev-swift (BUDGET_DEV_AGENTS member, outside NAMING) → budget-dev block injected, exit 0" {
  run_hook "glass-atrium-dev-swift"
  assert_status 0
  assert_ctx_contains "${BUDGET_DEV_NEEDLE}"
}
...
# (c3) budget fail-open: markers/file absent → no hard error, budget omitted, OTHER blocks emit.

@test "budget markers absent → fail-open exit 0, budget omitted, naming block still emits" {
  budget_src="${BATS_TEST_TMPDIR}/budget-no-markers.md"
  printf 'no budget markers here\n' >"${budget_src}"
  run_hook "glass-atrium-dev-front" "${NAMING_FIXTURE}" "${budget_src}"
  assert_status 0
  assert_ctx_contains "${NAMING_NEEDLE}"
  assert_ctx_not_contains "${BUDGET_DEV_NEEDLE}"
  # The fail-open path emits a single stderr diagnostic naming the empty source (bats merges it).
  [[ "${output}" == *"budget-dev block empty or markers absent"* ]] || {
    echo "expected budget-dev fail-open stderr diagnostic, got: ${output}" >&2
    return 1
  }
}

@test "budget source file absent entirely → fail-open exit 0, budget omitted, naming unaffected" {
  run_hook "glass-atrium-dev-front" "${NAMING_FIXTURE}" "/nonexistent/turn-budget.md"
  assert_status 0
  assert_ctx_contains "${NAMING_NEEDLE}"
  assert_ctx_not_contains "${BUDGET_DEV_NEEDLE}"
}
...
# and is ordered before the six droppable scope blocks so it survives the ~2KB persistence preview.
...
# (e) meter-first assembly + universal 9984-byte ceiling drop order (P1-T1 / P1-T2).
# Unlike run_hook (which suppresses the meter), these tests ENABLE it: they build a maxTurns
# frontmatter fixture + the scope-block sources, then assert the meter is assembled FIRST and the
# 9984-byte ceiling drops blocks in the pinned order lesson → budget-analysis → budget-dev → naming
# → style-ref → minimalism → comment-logging while never dropping the meter (dev-react carries no
# budget blocks, so its drop sequence starts at naming). Distinct needles per block make each
# assertion mutation-falsifiable.
...
# Build the block sources + a maxTurns frontmatter dir, then drive the hook with the meter
# ENABLED (SUBAGENT_BUDGET_METER_OFF unset, AGENTS_DIR pointed at the fixture). style_ref + minimalism
# share ONE source file (scope-dev.md) carrying both marker pairs, mirroring production; the two
# budget blocks likewise share ONE source file (shared-turn-budget.md). Pads default to a tiny size;
# a test overrides them to force ceiling drops (a budget block only lands when the agent is in its
# roster, so the budget pads are inert for dev-react).
# Args: $1=agent $2=comment_pad $3=styleref_pad $4=minimalism_pad $5=naming_pad
#       $6=budget_dev_pad $7=budget_analysis_pad
run_hook_full() {
  local agent="${1}" cpad="${2:-16}" spad="${3:-16}" mpad="${4:-16}" npad="${5:-16}"
  local bdpad="${6:-16}" bapad="${7:-16}"
  ...
  run bash -c '
    agent="$1"; hook="$2"; comment_src="$3"; styleref_src="$4"; naming_src="$5"; budget_src="$6"; agents_dir="$7"
    payload="$(jq -nc --arg a "${agent}" '\''{agent_type:$a}'\'')"
    printf "%s" "${payload}" | env \
      ...
      INJECT_SCOPE_RULES_LESSONS_SRC=/nonexistent \
      bash "${hook}"
  ' _ "${agent}" "${HOOK_SH}" "${comment_src}" "${styleref_src}" "${naming_src}" "${budget_src}" "${agents_dir}"
}
...
@test "small blocks under the 9984-byte ceiling — meter + all four scope blocks retained (P1-T2 baseline)" {
  run_hook_full "glass-atrium-dev-react"
  assert_status 0
  assert_ctx_contains "${METER_NEEDLE}"
  assert_ctx_contains "${COMMENT_NEEDLE}"
...
@test "over the 9984 ceiling by one block — naming dropped FIRST, meter + other three retained (P1-T2 order 1)" {
  # Pad sized so the four-block total clears 9984 by less than one block: dropping naming alone
...
@test "far over the 9984 ceiling — drops naming, style-ref, minimalism in order; keeps comment + meter (P1-T2)" {
  run_hook_full "glass-atrium-dev-react" 5000 5000 5000 5000
  assert_status 0
  assert_ctx_contains "${METER_NEEDLE}"
  assert_ctx_contains "${COMMENT_NEEDLE}"
...
@test "meter is NEVER dropped — a single oversized block cannot evict it (P1-T2 invariant)" {
  # comment alone = 10000B > the 9984 ceiling; after dropping naming/style-ref/minimalism the comment+meter
...
# (e2) budget-block drop order — the two budget blocks are the newest, least-proven droppable
...
@test "over the ceiling — budget-analysis sheds BEFORE naming; comment + naming + meter retained (qa-code-reviewer)" {
  # qa-code-reviewer receives comment + naming + budget-analysis (no style-ref/minimalism/budget-dev).
  # comment ~3619B (pad 3600) + naming ~3619B + budget-analysis ~827B (pad 800) + emit+meter ~2029B
  # ≈ 10102B > 9984 by ~118B (< the budget-analysis block size): dropping budget-analysis ALONE
  # restores the ceiling — naming MUST survive.
  run_hook_full "glass-atrium-qa-code-reviewer" 3600 16 16 3600 16 800
  assert_status 0
  assert_ctx_contains "${METER_NEEDLE}"
  assert_ctx_contains "${COMMENT_NEEDLE}"
  assert_ctx_contains "${NAMING_NEEDLE}"
