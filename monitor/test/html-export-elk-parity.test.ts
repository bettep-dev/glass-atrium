...
//
// 그 갈림이 닫힌 뒤에도 비교 대상은 index.html 하네스가 아니라 아래 뷰어 렌더 경로다. 그 경로는
// 나머지 입력(고정 드라이버 · 벤더 ELK 로더 · 공유 설정 · 문서 lang)을 내보내기가 주입하는 바로
// 그 파일에서 읽어 맞추므로, 대조에 남는 변수가 선언된 화면 기준 하나뿐이다.
//
